...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!area) return {};
  return buildMetadata({ title: `Teknisi Aki Darurat ${area.name} — 24 Jam`, description: `Butuh bantuan aki darurat di ${area.name}? ${business.name} melayani cek, jumper, ganti, dan pasang aki mobil maupun motor dengan teknisi express.`, path: `/area/${area.slug}` });
}
...
export default async function AreaPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <>
      <div className="bg-brand-dark text-white"><div className="mx-auto max-w-6xl px-4 py-10 md:py-14"><Breadcrumbs items={[{ label: "Area Jakarta" }, { label: area.name }]} /><h1 className="mt-4 max-w-3xl text-3xl font-extrabold tracking-tight text-balance md:text-4xl">Teknisi Aki Darurat {area.name} untuk Mobil &amp; Motor</h1><p className="mt-4 max-w-3xl text-sm leading-relaxed text-white/75 md:text-base">{area.description}</p><div className="mt-6 flex flex-col gap-2.5 sm:flex-row"><Button asChild size="lg" className="bg-brand-yellow text-brand-dark hover:bg-brand-yellow/90"><a href={waLink(waAreaMessage(area.name))} target="_blank" rel="noopener noreferrer"><WhatsAppIcon /> Panggil Teknisi dari {area.name}</a></Button><Button asChild size="lg" variant="outline" className="border-white/30 bg-transparent text-white hover:bg-white hover:text-brand-dark"><Link href="/produk">Lihat Layanan Aki</Link></Button></div></div></div>

      <div className="mx-auto max-w-6xl px-4 py-12 md:py-14"><div className="grid gap-10 lg:grid-cols-[1.4fr_1fr]"><div className="flex flex-col gap-4 text-sm leading-relaxed text-muted-foreground md:text-base"><h2 className="text-xl font-extrabold tracking-tight text-foreground">Aki Darurat di {area.name}? Kami Datang</h2><p>Bengkel kami berlokasi di {business.address}. {area.travelNote}</p><p>Sebelum teknisi berangkat, kirim tipe kendaraan dan lokasi melalui WhatsApp. Jika sesuai, aki dapat langsung dipasang oleh tim kami.</p><p>Belum tahu penyebab kendaraan sulit starter? Kirim merek, tipe, tahun kendaraan, dan lokasi melalui WhatsApp — kami bantu arahkan pemeriksaan untuk area {area.name}.</p></div><div className="flex flex-col gap-4 rounded-lg bg-white p-6 shadow-sm ring-1 ring-brand-dark/10"><h2 className="font-heading text-base font-extrabold tracking-wide uppercase">Lokasi &amp; Kontak</h2><ul className="flex flex-col gap-3.5 text-sm"><li className="flex items-start gap-2.5"><MapPinIcon className="mt-0.5 size-4 shrink-0 text-primary" aria-hidden /><a href={business.googleMapsUrl} target="_blank" rel="noopener noreferrer" className="font-semibold hover:underline">{business.address}</a></li><li className="flex items-start gap-2.5"><PhoneIcon className="mt-0.5 size-4 shrink-0 text-primary" aria-hidden /><a href={`tel:${business.phoneIntl}`} className="font-bold hover:underline">{business.phoneDisplay}</a></li><li className="flex items-start gap-2.5"><ClockIcon className="mt-0.5 size-4 shrink-0 text-primary" aria-hidden /><span>{business.hours[0]?.time}</span></li></ul><Button asChild className="mt-auto"><a href={business.googleMapsUrl} target="_blank" rel="noopener noreferrer"><MapPinIcon /> Buka Google Maps</a></Button></div></div></div>
