...
const trustItems = [
  ...
  { label: "Biaya transparan", icon: TagIcon },
];
const quickLinks = [
...
export default function HomePage() {
  return (
    <>
      ...
      <section className="mx-auto max-w-6xl px-4 py-12 md:py-20" id="tentang"><div className="grid gap-8 lg:grid-cols-[0.95fr_1.05fr] lg:items-center"><div className="relative overflow-hidden rounded-xl bg-white p-2 shadow-xl shadow-brand-dark/10"><div className="relative aspect-[4/3] overflow-hidden rounded-lg"><Image src={referenceMedia.profile.src} alt={referenceMedia.profile.alt} fill sizes="(max-width: 1024px) 100vw, 42vw" className="object-cover" /></div><div className="absolute right-6 bottom-6 rounded-lg bg-brand-dark px-4 py-3 text-white shadow-lg"><p className="text-[10px] font-black tracking-[0.16em] text-brand-yellow uppercase">Profil layanan</p><p className="mt-1 text-sm font-bold">Siap bantu di lokasi</p></div></div><div><Badge className="bg-brand-red px-2.5 py-1 text-[11px] font-black tracking-wide text-white uppercase">Tentang Kami</Badge><h2 className="mt-4 max-w-xl text-3xl font-black tracking-tight md:text-5xl">Aki soak tidak perlu membuat rencana Anda berhenti.</h2><p className="mt-5 max-w-2xl text-sm leading-relaxed text-muted-foreground md:text-base">{business.shortDescription}</p><div className="mt-7 grid gap-3 sm:grid-cols-3">{["Teknisi profesional", "Biaya transparan", "Konsultasi sebelum tindakan"].map((item) => <div key={item} className="flex items-start gap-2 pt-3 text-sm font-bold"><CheckIcon className="mt-0.5 size-4 shrink-0 text-brand-red" />{item}</div>)}</div><Button asChild className="mt-8 bg-brand-red text-white hover:bg-brand-red/90"><Link href="/tentang">Kenal Lebih Dekat <ArrowRightIcon /></Link></Button></div></div></section>
      ...
      <section className="mx-auto max-w-6xl px-4 py-12 md:py-20"><SectionHeading eyebrow="Kategori" title="Solusi aki untuk mobil dan motor" description="Pilih kategori kendaraan untuk melihat pilihan aki yang tersedia." /><CategoryCards /></section>
      <ServiceSupportGrid />
      <section className="bg-[#FAFAF8] py-12 md:py-20" id="produk"><div className="mx-auto max-w-6xl px-4"><div className="flex flex-col justify-between gap-4 md:flex-row md:items-end"><SectionHeading eyebrow="Produk" title="Aki yang tersedia" description="Pilihan aki referensi yang bisa dikonfirmasi dan dipasang oleh tim kami." /><Button asChild variant="outline" className="w-fit border-brand-dark/15 text-brand-dark hover:bg-brand-dark hover:text-white"><Link href="/produk">Lihat Semua Produk <ArrowRightIcon /></Link></Button></div><div className="mt-6 grid gap-4 sm:mt-8 sm:grid-cols-2 lg:grid-cols-4 lg:gap-5">{showcaseProducts.map((product) => <ProductShowcaseCard key={product.id} product={product} />)}</div></div></section>
      <section className="bg-[#F1F2EF] py-12 md:py-20"><div className="mx-auto max-w-6xl px-4"><SectionHeading eyebrow="Cara kerja" title="Aki bermasalah? Beres dalam 3 langkah" description="Proses singkat untuk kondisi kendaraan yang butuh penanganan segera." /><div className="mt-6 grid gap-3 sm:mt-8 sm:gap-4 md:grid-cols-3">{steps.map((step) => <div key={step.number} className="rounded-lg bg-white p-5 shadow-sm ring-1 ring-brand-dark/5 sm:p-6"><p className="text-4xl font-black text-brand-red">{step.number}</p><h3 className="mt-4 text-lg font-black sm:mt-5">{step.title}</h3><p className="mt-2 text-sm leading-relaxed text-muted-foreground">{step.description}</p></div>)}</div></div></section>
      <JakartaCoverage />
      <section className="bg-[#FAFAF8] py-12 md:py-20" id="dokumentasi"><div className="mx-auto max-w-6xl px-4"><div className="flex flex-col justify-between gap-4 md:flex-row md:items-end"><SectionHeading eyebrow="Dokumentasi" title="Galeri layanan aki" description="Bukti pemasangan pelanggan dan dokumentasi aktivitas teknisi di lapangan." /><a href={referenceSourceUrl} target="_blank" rel="noopener noreferrer" className="w-fit text-sm font-black text-brand-red hover:underline">Lihat sumber asli <ArrowRightIcon className="inline size-4" /></a></div><GalleryGrid /></div></section>
      <Testimonials />
      <section className="bg-white py-12 md:py-20"><div className="mx-auto max-w-6xl px-4"><SectionHeading eyebrow="Kepercayaan" title="Kenapa pelanggan memilih kami" description="Informasi layanan yang penting saat Anda membutuhkan aki dengan cepat." /><div className="mt-6 grid gap-3 sm:mt-8 sm:grid-cols-2 sm:gap-4 lg:grid-cols-4">{trustItems.map(({ label, icon: Icon }) => <div key={label} className="rounded-lg bg-[#FAFAF8] p-4 shadow-sm ring-1 ring-brand-dark/5 sm:p-5"><div className="flex size-10 items-center justify-center rounded-lg bg-brand-red/10 text-brand-red"><Icon className="size-5" aria-hidden="true" /></div><h3 className="mt-4 text-base font-black sm:mt-5">{label}</h3></div>)}</div></div></section>
      <section className="mx-auto max-w-6xl px-4 py-12 md:py-20" id="faq"><SectionHeading eyebrow="Pertanyaan umum" title="Sebelum memanggil teknisi" description="Jawaban singkat untuk pertanyaan yang paling sering ditanyakan pelanggan." /><div className="mt-6 sm:mt-8"><FaqSection items={faqs.slice(0, 6)} /></div></section>
...
function ProductShowcaseCard({ product }: { product: Product }) {
  ...
  return <Card className="group flex h-full flex-col overflow-hidden bg-white shadow-sm shadow-brand-dark/5"><div className="relative aspect-[4/3] overflow-hidden bg-white">{media ? <Image src={media.src} alt={media.alt} fill sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 25vw" className="object-contain p-4 transition-transform duration-300 group-hover:scale-105" /> : null}</div><CardContent className="flex flex-1 flex-col p-5 pb-3"><p className="text-[10px] font-black tracking-[0.16em] text-brand-red uppercase">{product.vehicleType === "mobil" ? "Aki Mobil" : "Aki Motor"}</p><h3 className="mt-2 text-lg font-black">{product.name}</h3><p className="mt-2 text-xs leading-relaxed text-muted-foreground">{product.shortDescription}</p><div className="mt-auto pt-5"><Button asChild size="sm" className="h-11 w-full bg-brand-red text-white shadow-sm shadow-brand-red/15 hover:bg-brand-red/90"><a href={waLink(waProductMessage(product))} target="_blank" rel="noopener noreferrer"><WhatsAppIcon /> Tanya &amp; Pesan</a></Button></div></CardContent></Card>;
}
